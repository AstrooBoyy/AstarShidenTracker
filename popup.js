...
var native_balance = document.getElementById("native_balance");
...
const chains = [
	{"title":"ASTAR", "nativeCoin":"ASTR", "id" : ASTAR_MAINNET_ID, "rpc": ASTAR_MAINNET_RPC, "type":"mainnet"},
	{"title":"Shiden Mainnet", "nativeCoin":"SDN", "id" : SHIDEN_MAINNET_ID, "rpc": SHIDEN_MAINNET_RPC, "type":"mainnet"},
	{"title":"Shiden Testnet (Shibuya)", "nativeCoin":"SDN", "id" : SHIDEN_TESTNET_ID, "rpc": SHIDEN_TESTNET_RPC, "type":"testnet"},
]

if(!localStorage.getItem('chainType') || !localStorage.getItem('chainID') || !localStorage.getItem('rpc')){
	localStorage.setItem('chainID', ASTAR_MAINNET_ID);
	localStorage.setItem('rpc', ASTAR_MAINNET_RPC);
	localStorage.setItem('chainType', "mainnet");
	localStorage.setItem('nativeCoin', "ASTR");
}
...
var nativeCoin = localStorage.getItem('nativeCoin');
...
function loadMoreNFT(targetElement, sendedData){
	...
	;(async () => {
		...
		for(var i=NftInList; i < maxToShow; i++){
			...
			let attributes = "";
			for(var j=0; j<resultNFTs.data.items[i].attributes.length; j++){
				attributes += `<div class="attribute"><span class="name">${resultNFTs.data.items[i].attributes[j].trait_type}:</span><br> ${resultNFTs.data.items[i].attributes[j].value}</div>`
			}
			...
			listNFT.innerHTML += `
				<li class="tokenNFT">
					...
					<div id="attributesNFT">${attributes}</div>
				</li>
			`;
...
async function getTokenBalance(chainId, address, sort_by, asc_desc) {
	...
	for(let i = 0; i < tokensContracts.length; i++){
	...
	newObject.nativeBalance = await getNativeTokenBalance(address);
...
async function sortTokenBalance(result, sort_by, asc_desc){
	...
	var newObject = {};
	newObject.data = {};
	newObject.nativeBalance = result.nativeBalance;
	newObject.error = false;
...
async function obtainUsersNfts(chainId, address){
	let web3 = new Web3(rpc);
	address = web3.utils.toChecksumAddress(address);
	...
	const contractAddress = web3.utils.toChecksumAddress("0x8b5d62f396Ca3C6cF19803234685e693733f9779");
	let contractInst = new web3.eth.Contract(contractAbi, contractAddress);
	let countNfts = await contractInst.methods.balanceOf(address).call()
	...
	for(var i = 0; i < countNfts; i++){
		p1.push(contractInst.methods.tokenOfOwnerByIndex(address, i).call())
	}

	const tokensIds = await Promise.all(p1);
	for(var i = 0; i < tokensIds.length; i++){
		p2.push(contractInst.methods.tokenURI(tokensIds[i]).call())
	}

	const arrUri = await Promise.all(p2);
	arrUri.map((uri, index)=>{
        p3.push(fetch(arrUri[index].replace("ipfs://", "https://hbd.infura-ipfs.io/ipfs/")));
    });

    const mtds = await Promise.all(p3);
    mtds.map((metadata, index)=>{
        p4.push(metadata.json());
    });


    const resArr = await Promise.all(p4);
    const newArray = [];
    resArr.map((item, index)=>{
        if(item.image){
        	item.image = item.image.replace("ipfs://", "https://hbd.infura-ipfs.io/ipfs/")
        	newArray.push(item);
        }
    });
...
function render(template_name, node, dataSended) {
    ...
    switch(template_name){
    	case "balance":
    		...
    		<p id="balance_total"><span id="native_balance">-</span> ${nativeCoin}</p>
	    	...
	    	var tokenListNew = "";
			native_balance = document.getElementById("native_balance");
    		...
    		;(async () => {
				...
				if (result.error) {
					...
					native_balance.innerHTML = `${result.nativeBalance.toFixed(4).toLocaleString()}`;
				}
				else{
					tokenList = "";
					var balance = null;
					...
					if(result.data.items.length > 0){
						listOfTokens.innerHTML = `<div class="erc20tokens">Tracked ERC-20 tokens:</div><span id="trackNewToken">Track new token</span>
						...
						listOfTokens.innerHTML = `<div class="erc20tokens">No tracked ERC-20 tokens</div><span id="trackNewToken">Track new token</span>
						<div id="trackNewTokenForm" class="hidden" >
		    				<input id="contractAddress" type="text" placeholder="Contract address" >
		    				<p id="trackError" style="margin-bottom: 5px;"></p>
		    				<div class="btns"  style="flex-direction: row;">
	    	...
	    	refreshAndSort = document.getElementById("refreshAndSort");
	          ...
	          <div id="sort_block">
	    		...
	    		if(dataSended.data.items.length > 5){
					...
					native_balance.innerHTML = `${parseFloat(dataSended.nativeBalance).toFixed(4).toLocaleString()}`;
    	...
    	case "nfts":
	    	...
	    	if(chainId != 592){
	    		listBlock.innerHTML = `
	    			<ul>
				    	<li class="error" >
				    		<p class="title" style=" font-weight: 400; ">This function is available now only for Astar Mainnet</p>
				    	</li>
			    	</ul> 
	    		`;
	    	} else{
	    		...
	    		;(async () => {
						resultNFTs = await obtainUsersNfts(chainId, address);
						...
						if (resultNFTs.error) {
						    ...
						    <div id="listBlock" style="margin-top:0px">
						    	<ul>
						    	<li class="error NFT">
						    		<p class="title">Error #${resultNFTs.error_code}:</p>
						    		<p class="subtitle">${resultNFTs.error_message}</p>
						    	</li>
						    	</ul>
						    </div> `;					    
						}
			    			...
			    			<ul>
						    	<li class="error NFT">
						    		...
						    		- <a target="_blank" href="https://blockscout.com/astar/token/0x8b5d62f396Ca3C6cF19803234685e693733f9779/token-transfers">Astar Cats</a>
						    	</li>
					    	</ul> 	
			    		`;
    		...
    		back.addEventListener("click", ()=>{
				...
				confirmAdd.addEventListener("click", ()=>{
					...
					if(!walletName.value.trim()){
						...
						arrayOfWallets.push({"name": walletName.value, "address":walletAddress.value.trim(), "erc20-tokens" : {"592":[], "336":[], "81":[]} });
						localStorage.setItem("walletsStorage", JSON.stringify(arrayOfWallets));
    	...
    	case "network":
    		...
    		for(const chain of chains){
    			...
    			divNetworks.innerHTML += ` 
	    			<li class="network ${active}" data-rpc="${chain.rpc}" data-nativeCoin="${chain.nativeCoin}" data-id="${chain.id}" data-type="${chain.type}">${chain.title}</li>
	    		`;
    		}
    		...
    		allNetworks.forEach((item) => {
					item.addEventListener("click", ()=>{
						...
						localStorage.setItem('nativeCoin', item.getAttribute("data-nativeCoin"));
						...
						nativeCoin = localStorage.getItem('nativeCoin');
...
function checkImage(tokenContractAddress, tokenContractName){
	const chainId = localStorage.getItem('chainID').toString();
	...
	if( tokensLogo[chainId].includes(tokenContractAddress.toLowerCase()) ){
		let index = tokensLogo[chainId].indexOf(tokenContractAddress.toLowerCase())
		return `./resources/images/token_logo/${chainId}/${tokensLogo[chainId][index]}.png`
	}
